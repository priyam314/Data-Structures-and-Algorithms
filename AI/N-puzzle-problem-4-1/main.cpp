...
int main() {
  ...
  vector<vector<int>>initial(sqrt(n+1),vector<int>(sqrt(n+1),0));
  vector<vector<int>>goal(sqrt(n+1),vector<int>(sqrt(n+1),0));

  cout<<initial.size()<<" "<<goal.size()<<" "<<length<<endl;
  cout<<" intial array elements "<<endl;
  ...
  cout<<"\n";
  if (process(initial,goal,checkx,checky,move)){
    cout<<"true"<<endl;
  }else{
    cout<<"false";
  } 
